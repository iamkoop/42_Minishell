...
/*   Updated: 2026/09/07 12:34:03 by nildruon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cleanup_vars_in_child_process(t_minishell *mini,
	t_single_linked_node	**envp, int err)
{
...
void	child_process(t_minishell *mini, t_single_linked_node	**envp,
		int close_read, int child_type)
{
	if (close_read)
	...
	{
		if (!final_redir(&mini->prev_read_fd, &mini->redir_in,
				child_type, STDIN_FILENO))
			cleanup_vars_in_child_process(mini, envp, 1);
	}
	if (mini->redir_out < -1 && (child_type != 2 && child_type != 3))
	{
		if (!final_redir(&mini->next_pipe_fds[1], &mini->redir_out,
