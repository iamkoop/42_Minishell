...
typedef struct s_exit_test
{
    ...
    char    **input;
    int     expected_status;
    char    *expected_stdout;   // Checked via strstr (usually "exit")
    char    *expected_err_msg;  // Checked via strstr (NULL if none)
}   t_exit_test;

/*
** Helper: Cleans up any resources allocated during child test state setup
** to prevent Valgrind from catching "still reachable" blocks on child termination.
*/
static void child_mem_cleanup(t_single_linked_node *lst)
{
    if (lst)
        ft_single_lstclear(&lst, del_env_node_content);
}

static int run_single_exit_test(t_exit_test test, t_single_linked_node *lst)
{
    int     out_pipe[2];
    int     err_pipe[2];
    pid_t   pid;
    ...
    if (pid == 0)
    {
        ...
        /*
        ** If builtin_exit encountered a non-fatal error (e.g., "too many arguments")
        ** it returns here instead of exiting. We clean up allocated copies
        ** explicitly before forcing exit to maintain a 100% leak-free footprint.
        */
        child_mem_cleanup(lst);
        exit(1);
    }
...
int test_builtin_exit(char **envp)
{
    ...
    /*
    ** Emulate your real minishell life cycle by translating the envp matrix 
    ** to your specific internal structures before building test scenarios.
    */
    ...
    char *case2[] = {"exit", "0", NULL};
    char *case3[] = {"exit", "42", NULL};
    char *case4[] = {"exit", "259", NULL};                 // Overflow: 259 % 256 = 3
    char *case5[] = {"exit", "-5", NULL};                  // Underflow: 256 - 5 = 251
    char *case6[] = {"exit", "9223372036854775807", NULL};  // LLONG_MAX -> Status 255
    char *case7[] = {"exit", "9223372036854775808", NULL};  // Out of range signed bounds -> Bash Status 2
    char *case8[] = {"exit", "not_a_num", NULL};           // Numeric argument check -> Bash Status 2
    char *case9[] = {"exit", "1", "2", NULL};               // Multi-argument check
    ...
    {
        if (!run_single_exit_test(tests[i], env_lst))
            success = 0;
    }

    // Clean up parent-side context safely
    child_mem_cleanup(env_lst);
    return (success);
}