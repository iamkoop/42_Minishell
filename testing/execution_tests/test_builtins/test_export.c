...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/06/29 19:30:18 by username          #+#    #+#             */
...
typedef struct s_export_test
{
	...
	int		setup_type;
	// 0: default, 1: single node list, 2: empty list
	char	*check_added_key;
	// Key to verify exists after export
	char	*check_added_value;
	// Value to verify match (NULL if key-only / no value)
}	t_export_test;
...
static int	run_single_export_test(t_export_test test, char **original_envp)
{
	...
	if (pid == 0)
	{
		...
		close(err_pipe[1]);
		int	actual_status = export(test.input, &env_lst);
		fflush(stdout);
		fflush(stderr);
		int	state_ok = 1;
		if (test.check_added_key)
			state_ok = verify_key_value(env_lst, test.check_added_key, test.check_added_value);
...
int	test_export(char **envp)
{
	...
	char	*case2[] =
	{
		"export", "NEW_VAR=hello", NULL
	};
	char	*case3[] =
	{
		"export", "KEY_NO_VALUE", NULL
	};
	char	*case4[] =
	{
		"export", "LONELY_VAR=mutated_value", NULL
	};
	char	*case5[] =
	{
		"export", "EMPTY_VAL=", NULL
	};
	char	*case6[] =
	{
		"export", "VAR_A=1", "VAR_B=2", "VAR_C=3", NULL
	};
	char	*case7[] =
	{
		"export", "invalid-id=123", NULL
	};
	char	*case8[] =
	{
		"export", "123var=true", NULL
	};
	char	*case9[] =
	{
		"export", "=value", NULL
	...
	char	*case11[] =
	{
		"export", "VALID_ONE=ok", "bad@id=no", "VALID_TWO=yes", NULL
	};
	t_export_test	tests[] =
	{
		{
	...
	{
		int	res = run_single_export_test(tests[i], envp);
		if (res == -1)
		{
			success = 0;
			break ;
