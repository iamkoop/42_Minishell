...
static int run_single_exec_test(t_exec_test *test, t_single_linked_node *envp)
{
    ...
    if (pid == 0)
    {
        ...
        while (curr_cmd_node)
        {
            ...
            if (cmd->redir == NULL)
            {
                printf("    redir: (NULL / No Redirections)\n");
            }
            else
            {
                t_single_linked_node *curr_redir_node = cmd->redir;
                int r_idx = 0;
                while (curr_redir_node)
                {
                    t_redir_list *r = (t_redir_list *)curr_redir_node->content;
                    printf("    redir[%d]: file='%s', type=%d, fd=%d\n", 
                           r_idx++, r->filename, r->redir_type, r->fd);
...
int test_general_cmd_exec(void)
{
    ...
    /* --------------------------------------------------------------------- */
    {
        static char *cat_argv3[] = {"cat", "exec_in1.txt", NULL};
        static char *grep_argv3[] = {"grep", "hello", NULL};
    ...
    /* --------------------------------------------------------------------- */
    {
        static char *cat_no_args[] = {"cat", NULL};
        static char *grep_argv4[] = {"grep", "hello", NULL};
